//
//  SupportingViewController.swift
//  rescat
//
...
class SupportingViewController: UIViewController {
    
    @IBOutlet weak var tableView: UITableView!
    
    var supports :[MyPageSupportData] = [MyPageSupportData]() {
        didSet {
...
extension SupportingViewController: UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return supports.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: SupportingTableViewCell.reuseIdentifier) as! SupportingTableViewCell
        ...
        let current = supports[indexPath.row].currentAmount
        ...
        print(self.progressInit(part: supports[indexPath.row].currentAmount, all: supports[indexPath.row].goalAmount))
...
extension SupportingViewController {
    
    //내가 작성한 후원 글 조회
    func getSupportList() {
        
        GetMySupportService.shareInstance.getSupportList(completion: {
            (result) in
